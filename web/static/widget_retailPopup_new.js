...
var serverPath, cdnPath, $, cwAppUrl="", masterCssFilePath, cssFilePath, cwOverlayCssClass, cwStructureCss, cwConditionalCss = [], cwWidgetWrapCss, loadCheck = 1, cwData, cwButtonText, cwContainerClass, customData = 0, cwTwoOverlay = [], cwVersion, checkRefresh = 1, chkThemeID = '', variantChngChk = 1, cwWV = navigator.userAgent.indexOf('cw_WV');
...
function getChildUrlData(url, cwId, ip, clientUrl, retailerId, productId, productName, category, productPrice,productVariant,retailerName,quantity,cwPriSym ,productEan) {
...
function CTGetWidget(proId,CampaignID,classSelectorObj) {
   // alert(proId);
 //  trackComponentInfo(classSelectorObj)
    ...
    function ready() {
        ...
        $(document).ready(function() {            
            loadDataWidget(compUrl, clientUrl,proId,CampaignID);
        }).scroll(function() {
    ...
    function horizontalL(slide){
        if(countryCode == '0'){
            ...
            if(slide){
                //$('.hr_wrapper').slideDown(300);
            }else{
                //$('.hr_wrapper').slideUp(300);
            }
        }
    }
    makeWidgetStructure = function(vInx,proId) {
        ...
        //     }   
        // }   
        for(r=0; r<cwData.data.CampaignProductItem.productItem.length;r++){
          
            for(k=0; k<cwData.data.CampaignProductItem.productItem[r].smartProduct.length;k++)
            {
              if(cwData.data.CampaignProductItem.productItem[r].smartProduct[k].smartProductId == proId)
             {
               cwJsonVal = cwData.data.CampaignProductItem.productItem[r].smartProduct[k].product;
             }
            }
        ...
        if ((cwJsonVal.length && loadCheck == 1) || cwMode == 1 || cwCountryChk) {
            ...
            for (i = 0; i < cwJsonVal.length; i++) {
                ...
                cwTitle = $.trim(cwJsonVal[i].widget_title);
                cwPreTitle = $.trim(cwJsonVal[i].pre_title_text);
                productName.push(cwJsonVal[i].product_name);
                productVariant.push(cwJsonVal[i].product_variant); 
                cwPostTitle = $.trim(cwJsonVal[i].post_title_text);
	           ...
	           if (cwJsonVal[i].cw_conditional_css.indexOf(',') >= 0) {
                ...
                if ($('#customFields').length) {
                    $('#customFields').val(cwJsonVal[i].custom_fields);
                }
        ...
        //   cwDigitalData("", "shop_now", arr);
                    ...
                    for (var v = 0; v < totalCnt; v++) {    
                        if(variantChngChk == 1 && cwCountryChk == 0){  
                            ...
                            if(variantCount > 1){                                
                                ...
                                for (var n = 0; n < variantCount[v]; n++) {
                                    if (variantWeight[n].length >= 35) {
                                        trimTxt = $.trim(variantWeight[n]).substring(0, 35).trim(this) + "...";
                                    }
                            ...
                            for (j = 0; j < cntCount[v]; j++) {
                                imgURL = retailerLogoMain[i][cwICount], imgLink = "getChildUrlData('" + buyUrlMain[i][cwICount] + "','" + proId + "','" + myip + "','" + clientUrl + "','" + retailerIdMain[i][cwICount] + "','" + productId[i] + "','" + widgetName[i] + "','" + cwCategoryMain[i][cwICount] + "','" + cwPriceMain[i][cwICount] + "','" + variantWeight[cwSelInx] + "','" + retailerNameMain[i][cwICount] + "','" + cwQuantityMain[i][cwICount] + "','" + cwPriceSym[v] + "','" + cwProductEan  + "');";
                            ...
                            /* Overlay */
                            $('body').append(js_builddata_divblock);
                            ...
                            $('.cw_widget_size_btn.cwObj').click(function(){ 
                            	if($('.cwObj.cwTabsBtn').eq(1).hasClass('active') && $('#cwStoreList li').length){
                            		cwGetStorePro($(this).attr('data-ean'));
	                            }
                            });
                            $('#cwMapSearch').keyup(function(){ 
                            	$('.search_btn.cwObj').addClass('search_disabled_btn');
                                cwSearchCallChk = 1;
                                // if($('#cwMapSearch').val().trim() != ''){
                                //    //$('.search_btn.cwObj').removeClass('search_disabled_btn');
                                // }else{
                                //     $('.search_btn.cwObj').addClass('search_disabled_btn');
                                // }
                            }); 
                            ...
                            cwUnAvailChk = '<table><tbody>';
                            for (j = 0; j < cntCount[v]; j++) {
                                imgURL = retailerLogoMain[i][cwICount], imgLink = "getChildUrlData('" + buyUrlMain[i][cwICount] + "','" + CARTWIREWIDGETKEYFORCHECKING + "','" + myip + "','" + clientUrl + "','" + retailerIdMain[i][cwICount] + "','" + productId[i] + "','" + widgetName[i] + "','" + cwCategoryMain[i][cwICount] + "','" + cwPriceMain[i][cwICount] + "','" + variantWeight[cwSelInx] + "','" + retailerNameMain[i][cwICount] + "','" + cwQuantityMain[i][cwICount] + "','" + cwPriceSym[v]  + "','" + cwProductEan +"'  );";
                                if (retailerNameMain[i][cwICount]) {
                                    if (isAvailMain[i][cwICount] == 1 && statusMain[i][cwICount] != 2) {
                                        cwUnPrice = cwPriceSym[v]+' <span class="getPrice">'+cwPriceMain[i][cwICount]+'</span>';
                                        if(cwPriceMain[i][cwICount] == "null"){
                            ...
                            stockRow = cwAvailChk+cwUnAvailChk;
                            // Tab content ends                            
                            $('#cwRetailsTbl').html(stockRow);
                        }
                        if(variantChngChk != 1){
                            $('.cw_widget_size_btn').removeClass('selected');
                            $('.cw_widget_size_btn').eq(vInx-1).addClass('selected');
                        }else{ 
                            $('.cw_widget_size_btn').eq(variantDefault-1).addClass('selected');
                        }
                        $('#cwRetailTbl th:nth-child(2)').unbind('click');
                        $('#cwRetailTbl th:nth-child(2)').click(function(){
                            $(this).toggleClass('ASC DESC');
                            $(this).children('#cwUpDownIcon').removeClass('shorting_down').addClass('shorting_up');
                            var thOrder = -1;
                                if($(this).hasClass("ASC")){
                                    thOrder = 1;
                                    $(this).children('#cwUpDownIcon').removeClass('shorting_up').addClass('shorting_down');
                                }
                            sortTable(document.getElementById("cwRetailTbl"),$(this).index(), thOrder);
                        ...
                        $('.cw_widget_buy_btn, .cw_widget_view_btn').on('keydown', function (e) {
                            if (e.which == 32) {                            	
                                $(this).trigger('click');                                
            ...
            $('.contact_btn.cwObj').on('keydown', function (e) {
                if (e.which == 32) {                            	
                    $(this).trigger('click');                                
					window.open($(this).attr('href'))
                    return false;
            ...
            $('.cwTabsBtn, .open_in_map_btn.cwObj, .use_current_location.cwObj').on('keydown', function (e) {
                if (e.which == 32){ 
                    $(this).trigger('click');
                }
            }); 
            $('.search_btn.cwObj').on('keydown', function (e) {            	
                if (e.which == 32 || e.which == 13){ 
                	if(!$(this).hasClass('search_disabled_btn')){
	                    $(this).trigger('click');	                    
	                }else{
	                	return false;
	                }
            ...
            $('.show_hide_map.cwObj').on('keydown', function (e) {
                if (e.which == 32){
                    $(this).trigger('click');
                    setTimeout(function(){
                        $('.show_hide_map.cwObj').focus();
                    },600)
                }
            }); 
            $('.cw_widget_size_btn').on('keydown', function (e) {
	            if (e.which == 32 || e.which == 13) {                            	
	                var cwThis = $(this);
	                $(this).trigger('click');
	                setTimeout(function(){
            ...
            $(document).on('keydown', function(event) {
                if (event.which == 32) {
                    if((document.activeElement.className).indexOf("cwCountryDrop") > -1){
                        $(".cwCountryDrop").trigger("mousedown");
                    }
                ...
                if (event.which == 9) {
                	if(event.shiftKey) {
		            	...
		            	if((document.activeElement.className).indexOf("boxClose1") > -1){
	                		$('.cw_product_img_wrap').focus();
		                    return false;
		                }
                        if($('.tabs_list .cwTabsBtn').eq(0).hasClass('active')){
            ...
            if(discOutStock.split(",")[cwSelInx] == '0'){
                ...
                cwDisClaimCont += '</div>';
                $('#cwDisclaim').html(cwDisClaimCont).show();
            }else{
                if (cwDisclaimerTxt != '') {
            ...
            var cssListener = setInterval(function() {
                $('#siteLoader_cw').fadeOut(100);
                $('#dialog_popup_inner_cw').fadeIn(100);
                if (variantDefault && checkRefresh) {
                    checkRefresh = 0;
                    /*if(variantChngChk){
                        $('#dialog_popup_inner_cw #cwVariantSelect option').eq(variantDefault-1).attr('selected', 'true');
                    }*/
                }
            ...
            loadChk = 1;
            setTimeout(function() { 
                $('.closebutton_cw, .closebutton_cont_cw, .' + cwStructureCss[0]).click(function(event) { closes(event) });
            }, 500);
        }
        widgetInCenter();
        countryChk = 0;
    ...
    cwSetIndex = function(){
        $('.cwObj').each(function(i){
            $(this).attr('tabindex', i+1);
        });
        $('.boxClose1').attr('tabindex', $('.cwObj').length+1);
    }
    getCountryRetialer = function(e){
        $('#cwDisclaim').hide();
        $('#siteLoader_cw').fadeIn(100); 
        loadCheck = 1;
        countryCode = $('#cwCountrySelect').val();
        clientUrl  = encodeURIComponent(document.location.href); 
        //closes(e,1); 
        setTimeout(function(){                  
            countryChk = 1;
    ...
    iPadMakeWidgetStructure = function(rInx) {
        ...
        setTimeout(function() {
            ...
            if($('.cw_widget_size_btn').eq(rInx).hasClass('selected')){
            	cwVarName = $('.cw_widget_size_btn').eq(rInx).attr('title');
            	cwVarID = $('.cw_widget_size_btn').eq(rInx).attr('data-varid');
    ...
    cwTabChange = function(cwTabs){
    	if(cwTabs == "1"){
            if(!$('.cwObj.cwTabsBtn').eq(1).hasClass('active')){
                cwAddViewCall('buyIn');
            }
    		$('.cwTabsBtn').eq(1).addClass('active');
    		$('.cwTabsBtn').eq(0).removeClass('active');
    		$('.store_wrap').fadeIn();
    		$('.table_gride').hide();
            if($('#cwStoreList').is(':visible')){ 
                $('#cwStoreList').animate({
                    scrollTop: 0
                },0);
            }            
    		...
    		$('.cwTabsBtn').eq(0).addClass('active');
    		$('.cwTabsBtn').eq(1).removeClass('active');
    		$('.store_wrap').hide();
    		$('.table_gride').fadeIn();
    ...
    cwOpenInMap = function(i,lat,long){
        $('.show_hide_map').eq(0).hide();
        $('.show_hide_map').eq(1).show();
        $('.cwGMap').slideDown(); 
    ...
    cwStoreListClick = function(iVar){        
    	$('#cwStoreList li').removeClass('active');
		$('#cwStoreList li').eq(iVar).addClass('active');
        ...
        //},400);		
		window.scrollTo({top: $('.your_nearest_stores').offset().top, behavior: 'smooth'});
    }
        ...
        $.ajax({    
            ...
            success: function(cwGPData) {                      
                if(cwGPData.Items.length == 0){       
                        ...
                        cwGPStData[i] = {SN:cwGPData.Items[i].StoreName, SA:cwGPData.Items[i].Street, SC:cwGPData.Items[i].City, PC:cwGPData.Items[i].PostalCode, SP:cwGPData.Items[i].PhoneNumber,SLT:cwGPData.Items[i].Lat,SLG:cwGPData.Items[i].Long}
        					...
        					google.maps.event.addListener(cwMarker, 'click', (function(marker, i) {
        				        return function() {
        							...
        							$('#cwStoreList li').removeClass('active');
        							$('#cwStoreList li').eq(i).addClass('active');
        				        }
        				    })(cwMarker, i));
                    ...
                    cwMaps.fitBounds(cwBounds); 
                    cwMaps.panToBounds(cwBounds);
    				$('#cwStoreList').html(stockGPRow).removeAttr("style").animate({
                        scrollTop: 0
                    },0);
                    $('.cwStRows.cwObj, .open_in_map_btn.cwObj').on('keydown', function (e) {
		                if (e.which == 32){
		                    $(this).trigger('click');
		                }
		            });
                    ...
                    $('.left.your_nearest_stores').html(cwResTxt).attr('aria-label',cwResTxt);
                    //
    				cwSetIndex();
    		        $('#cwMapSH').show();
    		        //$('.cwGMap').slideDown();
                    $('#cwNoStore').slideUp();
    		        $('#cwStoreList').slideDown();
        ...
        cwGeocoder.geocode({ location: latlng }, function(results, status)  {
            if (status === "OK") {
              if (results[0]) {
                ...
                if(gInx==2){
	                $('#cwMapSearch').val(results[3].formatted_address);
	            }
	            if(gInx!=2){
	...
	cwGetLocation = function(gInx){  
        if(gInx == 1){      
            if($('#cwMapSearch').val().trim() && !$('.search_btn').hasClass('search_disabled_btn')){
                /*var cwSrcStr = '';
                for(i=0; i<$('#cwMapSearch').val().split(/ +/).length; i++){
                    cwSrcStr += $('#cwMapSearch').val().split(/ +/)[i]+'+';
                }*/
                if(cwCurPosClient){  
                ...
                cwAddStr = cwCurPos.lat+'^'+cwCurPos.lng+'^'+$('#cwMapSearch').val().trim();
                if(cwSearchCallChk){
...
//
	...
	//Access Current Location Starts
    //
    function loadDataWidget(compUrl, clientUrl,proId,campaignID) {
        var cwConutDef = countryCode;
        if(countryCode){
            $.each(countryList.countries,function(key,val){
                if(val.default == 1){
                    cwConutDef = val.id;
                    return false;
                }
        ...
        var idmain="imported-CampaignProductItem-"+campaignID;
      //  var query = "query camapaignJSON ($idmain: ID!){Postitem(id: $idmain ){items{smartProduct{smartProductId product{product_id retailer{retailer_id}countryCount{country_name}}}}}}";
       // var query = "query {allProduct{EAN}}";
       var query=`query camapaignJSON ($idmain: ID!)
       {
        CampaignProductItem(id: $idmain )
       {
        productItem
        {
          smartProduct
          {
            smartProductId
              product
          {
            show_country_price_status
            show_country_offer_status
            show_store_locator
            table_header
            price_symbol
            pdate
            ...
            variant_default
            disclaimer
            disclaimer_out_stock1
            disclaimer_out_stock2
            disclaimer_status
            retailer
            { prtId
              iframe_url
    ...
    //     $.ajax({
    //     type: "post",
    //     url: 'https://ancmzeus.api.sanity.io/v1/graphql/production/default',
    //     data: JSON.stringify({
    //         query,
    //         variables: { idmain },
    //       })
    //     ,
    //    // data: JSON.stringify({"query": query}, variables:{ dice}),
    ...
    // });        
        var apiURL = 'https://magnum-campaign.netlify.app/myfile.json'; 
        
        horizontalL(0);        
        $('#dialog_popup_inner_cw .widgetScrollWrap').slideUp(600);
        // $.ajax({
        //     url: apiURL,
        //     async: true,
        //     type: "GET",
        ...
        $.ajax({
            type: "post",
            url: 'https://8gjfptsf.api.sanity.io/v1/graphql/production/default',
            data: JSON.stringify({
                query,
                variables: { idmain },
              })
    ...
    function createTemplate(compUrl, myip, widgetId, titlesNm) {
        ...
        if (variantChngChk) {
            ...
            if (cwMode) {
                $('#widgetPre').append(js_builddata_divblock + js_endblock);
            } else if ($('#cwWidgetEmb').length) {
                $('#cwWidgetEmb').append(js_builddata_divblock + js_endblock);
            } else {
                $('body').append(js_builddata_divblock + js_endblock);
            }
            ...
            setTimeout(function() {
                $('.closebutton_cw, .closebutton_cont_cw, .cwOuterPosition, .' + cwStructureCss[0]).click(function(event) { closes(event); });
            }, 500);
        }
    }
    closes = function(e,esc){ 
        ...
        if (cwElem == cwStructureCss[1] || cwElem.indexOf(cwConditionalCss[0]) > 0 || cwElem.indexOf('closebutton_cw') != -1 || cwElem.indexOf('closebutton_cont_cw') != -1 || cwElem.indexOf('cwOuterPosition') != -1 || esc == '1' || esc =='5') {
            jsVals = 0;
            if ($('#cwCustomStyle').length) {
                $('#cwCustomStyle').remove();
            }
            //$('#siteLoader_cw, #cwMainDivContainer, .' + cwStructureCss[0]).remove();
            $('#siteLoader_cw, #cwMainDivContainer, .cwOuterPosition, .pac-container.pac-logo, .' + cwStructureCss[0]).remove();                
            if (typeof(btnDisable) != "undefined") {
                btnDisable.disabled = false;
            }
            //$("link[href='" + cdnPath + "/v2.0/css/" + masterCssFilePath + "']").remove();
            /*if (cssFilePath) {
                $("link[href='" + cdnPath + "/v2.0/css/" + cssFilePath + "']").remove();
            }*/
            checkRefresh = 1;
            /*setTimeout(function(){                    
                $(cwBtnObject).focus();
    ...
    function widgetInCenter() {
        setTimeout(function() {
            ...
            $('#cwMainDivContainer').css({ 'top': $(window).scrollTop(), 'position': 'absolute' });
        }, 0);
    }
}